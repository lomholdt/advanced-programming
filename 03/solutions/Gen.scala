...
case class Gen[A] (sample :State[RNG,A]) {
  ...
  def listOfN (n :Int) :Gen[List[A]] =
    Gen(State.sequence[RNG,A] (List.fill (n) (this.sample)))
  ...
  def flatMap1[B] (f: A => Gen[B]) :Gen[B] =
    Gen(State(rng =>
        sample.run (rng) match { case (a,rng1) => f(a).sample.run (rng1) }))

  // this is the ultimate solution if you recall that we already have flatMap
  // for us which exactly interleaves two automata over the same state space.!

  def flatMap[B] (f: A => Gen[B]) :Gen[B] =
    Gen(sample.flatMap (a => f(a).sample))

  // It would be convenient to also have map
...
object Gen {
  ...
  def choose (start :Int, stopExclusive :Int) :Gen[Int] =
    Gen[Int]( State(RNG.nonNegativeInt).map
  ...
  // one solution
  def boolean1 :Gen[Boolean]  =
    Gen[Boolean] (State[RNG,Int](_.nextInt).map (n => n % 2 == 0))

  // Another solution.
  // This seems to be slightly more elegant, and it does proper reuse (Ex. 8.5)
  ...
  def double :Gen[Double] = Gen (State[RNG,Double](RNG.double))
  ...
  def weighted[A](g1: (Gen[A],Double), g2: (Gen[A],Double)): Gen[A] = {
    val pivot = g1._2 / (g1._2+g2._2)
    double.flatMap { d => if (d <= pivot) g1._1 else g2._1 }
  }
  //
...
case class Prop (run :(TestCases,RNG) => Result) {
  ...
  def && (that :Prop) :Prop = Prop {
    (tcs,rng) => run (tcs,rng) match {
      case r => if (r.isFalsified) r else that.run(tcs,rng)
    }
  }

  def || (that :Prop) :Prop = Prop {
    (tcs,rng) => run (tcs,rng) match {
      case r => if (r.isFalsified) that.run(tcs,rng) else r
    }
  }
