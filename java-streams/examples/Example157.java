// Example 157 from page 127 of Java Precisely third edition (The MIT Press 2016)
...
class Example157 {
  public static void main(String[] args) {
    ...
    Stream<String> numerals 
      = LongStream.iterate(0, x -> x+1).mapToObj(Example157::toEnglish);
    ...
    System.out.println("\nFive ways to compose Example157::toEnglish and String::length:");
    int enough = 35;
    Supplier<Stream<Long>> nats = () -> LongStream.range(1, enough).boxed();
    nats.get().map(Example157::toEnglish).map(String::length).forEach(x -> System.out.printf("%d ", x));
    System.out.println();
    // Illegal: The method reference expression Example157::toEnglish
    // must appear in assignment context, invocation context, or
    // casting context:
    // nats.get().map(Example157::toEnglish.andThen(x -> length(x))).forEach(x -> System.out.printf("%d ", x));
    traverse1(nats.get(), Example157::toEnglish, String::length).forEach(x -> System.out.printf("%d ", x));
    System.out.println();
    traverse2(nats.get(), Example157::toEnglish, String::length).forEach(x -> System.out.printf("%d ", x));
    System.out.println();
    traverse3(nats.get(), Example157::toEnglish, String::length).forEach(x -> System.out.printf("%d ", x));
    System.out.println();
    wildcardDemo();
  }
  ...
  private static final LongFunction<String> 
    less1K = less(          100, "hundred",  Example157::less100),
