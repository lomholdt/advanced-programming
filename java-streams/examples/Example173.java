// Example 173 from page 137 of Java Precisely third edition (The MIT Press 2016)
...
// fourty seven million four hundred eighty three thousand six hundred
// fourty eight
...
class Example173 {
  public static void main(String[] args) {
    ...
    Stream<String> numerals 
      = LongStream.iterate(0, x -> x+1).mapToObj(Example173::toEnglish);
    ...
    System.out.println("\nFive ways to compose Example173::toEnglish and String::length:");
    int enough = 35;
    Supplier<Stream<Long>> nats = () -> LongStream.range(1, enough).boxed();
    nats.get().map(Example173::toEnglish).map(String::length).forEach(x -> System.out.printf("%d ", x));
    System.out.println();
    // Illegal: The method reference expression Example173::toEnglish
    // must appear in assignment context, invocation context, or
    // casting context:
    // nats.get().map(Example173::toEnglish.andThen(x -> length(x))).forEach(x -> System.out.printf("%d ", x));
    traverse1(nats.get(), Example173::toEnglish, String::length).forEach(x -> System.out.printf("%d ", x));
    System.out.println();
    traverse2(nats.get(), Example173::toEnglish, String::length).forEach(x -> System.out.printf("%d ", x));
    System.out.println();
    traverse3(nats.get(), Example173::toEnglish, String::length).forEach(x -> System.out.printf("%d ", x));
    System.out.println();
    wildcardDemo();
  }
  ...
  private static final LongFunction<String> 
    less1K = less(          100, "hundred",  Example173::less100),
