// Example 158 from page 129 of Java Precisely third edition (The MIT Press 2016)
...
class Example158 {
  public static void main(String[] args) {
    ...
    Stream<String> numerals 
      = LongStream.iterate(0, x -> x+1).mapToObj(Example158::toEnglish);
    ...
    System.out.println("\nFive ways to compose Example158::toEnglish and String::length:");
    int enough = 35;
    Supplier<Stream<Long>> nats = () -> LongStream.range(1, enough).boxed();
    nats.get().map(Example158::toEnglish).map(String::length).forEach(x -> System.out.printf("%d ", x));
    System.out.println();
    // Illegal: The method reference expression Example158::toEnglish
    // must appear in assignment context, invocation context, or
    // casting context:
    // nats.get().map(Example158::toEnglish.andThen(x -> length(x))).forEach(x -> System.out.printf("%d ", x));
    traverse1(nats.get(), Example158::toEnglish, String::length).forEach(x -> System.out.printf("%d ", x));
    System.out.println();
    traverse2(nats.get(), Example158::toEnglish, String::length).forEach(x -> System.out.printf("%d ", x));
    System.out.println();
    traverse3(nats.get(), Example158::toEnglish, String::length).forEach(x -> System.out.printf("%d ", x));
    System.out.println();
    wildcardDemo();
  }
