...
class Example154 {
  public static void main(String[] args) {
    ...
    myList = cons(1, cons(5, cons(3, cons(4, cons(5, empty)))));
    FunList<FunList<T>> myList2 = ;
      // System.out.println(myList);
    System.out.println(myList.removeFun(5)); // 1,3,4
    System.out.println(myList.count(x -> x > 4)); // 2
    System.out.println(myList.filter(x -> x > 3)); // 5,4,5
...
class FunList<T> {
  ...
  public Node<T> remove(T x, Node<T> xs){
    if(xs.next == null && xs.item == x) return null;
    if(xs.next == null) return new Node<T>(xs.item, null);
    if(xs.next != null && xs.item == x) return remove(x, xs.next);
    if(xs.next != null) return new Node<T>(xs.item, remove(x, xs.next));
    return null;
  }
  ...
  public int count(Predicate<T> p, int count, Node<T> xs){
    if(xs.next == null){ // end of list
      if(p.test(xs.item)) return count+1;
      return count;
    }
    else{
      if(p.test(xs.item)){ 
        return count(p, count+1, xs.next);
      }
      else{
        return count(p, count, xs.next);
      }
    }
  }
  ...
  public Node<T> filter(Predicate<T> p, Node<T> xs){
    if(xs.next == null){ // end of list
